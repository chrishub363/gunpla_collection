...
| `--refresh` | Run the full pipeline — `scalemates:export` (opens a browser to log in) → `enrich:kits` → `kit_images:fetch` → `kit_images:prune` → reseed — refreshing the committed dataset from ScaleMates |
